...
describe('api functions', function()
  ...
  it("use buffer numbers as buffer handle", function()
    local nr = eval("bufnr('')")
    local hnd = eval("nvim_get_current_buffer()")
    eq(nr, hnd)
    execute("new")
    local nr2 = eval("bufnr('')")
    local hnd2 = eval("nvim_get_current_buffer()")
    eq(nr2, hnd2)
    neq(nr, nr2)
    -- 0 is synonymous to the current buffer
    eq(nr2, eval("nvim_buf_get_number(0)"))
  end)

  -- TODO(bfredl): When window ids get implemented
  -- they should tested to be equal to window handles here.
