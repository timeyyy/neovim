...
local function check_undo_redo(buf, ns, mark, sr, sc, er, ec) --s = start, e = end
  feed("u")
  rv = buffer('get_mark_index', buf, ns, mark)
  eq(sr, rv[2])
  eq(sc, rv[3])
  feed("<c-r>")
  rv = buffer('get_mark_index', buf, ns, mark)
...
describe('Extmarks buffer api', function()
  ...
  before_each(function()
    ...
    if ns == 0 then
      ...
      eq(1, ns)
      -- TODO(timeyyy): Test error
      -- rv = buffer('nvim_init_mark_ns', ns_string)
      -- eq(0, rv)
  ...
  it('adds, updates  and deletes marks #extmarks', function()
    ...
    eq(1, rv)
    -- rv = buffer('get_mark_index', buf, ns, marks[1])
    rv = buffer('get_mark_index', buf, ns, marks[1])
    ...
    rv = buffer('set_mark', buf, ns, marks[1], positions[1][1], positions[1][2])
    eq(2, rv)
    rv = buffer('get_mark_index', buf, ns, marks[2])
  ...
  it('querying for information and ranges #extmarks', function()
    ...
    rv = buffer('get_mark_ids', buf, ns)
    for i, m in ipairs(marks) do
        eq({m, positions[i][1], positions[i][2]}, rv[i])
    end

    -- next with mark id
    rv = buffer('get_next_mark', buf, ns, marks[1])
    eq({marks[2], positions[2][1], positions[2][2]}, rv)
    rv = buffer('get_next_mark', buf, ns, marks[2])
    eq({marks[3], positions[3][1], positions[3][2]}, rv)
    rv = buffer('get_next_mark', buf, ns, marks[3])
    eq({}, rv)
    -- next with positional when mark exists at position
    rv = buffer('get_next_mark', buf, ns, positions[1])
    eq({marks[2], positions[2][1], positions[2][2]}, rv)
    -- next with positional index (no mark at position)
    rv = buffer('get_next_mark', buf, ns, {positions[1][1], positions[1][2] +1})
    eq({marks[2], positions[2][1], positions[2][2]}, rv)
    -- TODO(timeyyy): test error?  test with extremity index
    -- rv = buffer('get_next_mark', buf, ns, -1)
    -- eq({marks[2], positions[2][1], positions[2][2]}, rv)

    -- nextrange with mark id
    rv = buffer('get_next_markrange', buf, ns, marks[1], marks[3])
    ...
    rv = buffer('get_next_markrange', buf, ns, positions[1], positions[3])
    eq({marks[1], positions[1][1], positions[1][2]}, rv[1])
    eq({marks[2], positions[2][1], positions[2][2]}, rv[2])
    eq({marks[3], positions[3][1], positions[3][2]}, rv[3])
    rv = buffer('get_next_markrange', buf, ns, positions[2], positions[3])
    eq(2, table.getn(rv))
    ...
    rv = buffer('get_next_markrange', buf, ns, lower, upper)
    eq({{marks[2], positions[2][1], positions[2][2]}}, rv)
    ...
    -- prev with mark id
    rv = buffer('get_prev_mark', buf, ns, marks[3])
    eq({marks[2], positions[2][1], positions[2][2]}, rv)
    rv = buffer('get_prev_mark', buf, ns, marks[2])
    eq({marks[1], positions[1][1], positions[1][2]}, rv)
    rv = buffer('get_prev_mark', buf, ns, marks[1])
    eq({}, rv)
    -- prev with positional when mark exists at position
    rv = buffer('get_prev_mark', buf, ns, positions[3])
    eq({marks[2], positions[2][1], positions[2][2]}, rv)
    -- prev with positional index (no mark at position)
    rv = buffer('get_prev_mark', buf, ns, {positions[1][1], positions[1][2] +1})
    eq({marks[1], positions[1][1], positions[1][2]}, rv)
    -- TODO test with positional index (failure)
    -- TODO test with extremity index (failure)
    -- prevrange with mark id
    rv = buffer('get_prev_markrange', buf, ns, marks[1], marks[3])
    ...
    rv = buffer('get_prev_markrange', buf, ns, positions[1], positions[3])
    eq({marks[3], positions[3][1], positions[3][2]}, rv[1])
    eq({marks[2], positions[2][1], positions[2][2]}, rv[2])
    eq({marks[1], positions[1][1], positions[1][2]}, rv[3])
    rv = buffer('get_prev_markrange', buf, ns, positions[1], positions[2])
    eq(2, table.getn(rv))
    -- prevrange with positional index (no mark at position)
    lower = {positions[2][1], positions[2][2] + 1}
    upper = {positions[3][1], positions[3][2] + 1}
    rv = buffer('get_prev_markrange', buf, ns, lower, upper)
    eq({{marks[3], positions[3][1], positions[3][2]}}, rv)
    lower = {positions[3][1], positions[3][2] + 1}
    upper = {positions[3][1], positions[3][2] + 2}
    rv = buffer('get_prev_markrange', buf, ns, lower, upper)
  ...
  pending('marks move with char inserts #manualgood', function() -- FIXME
    -- ins_char in misc1.c
    feed('a<cr>12345<esc>')
  ...
  it('namespaces work properly #extmarks', function()
    ...
    rv = buffer('get_mark_ids', buf, ns)
    eq(1, table.getn(rv))
    rv = buffer('get_mark_ids', buf, ns2)
    eq(1, table.getn(rv))
    ...
    rv = buffer('get_prev_mark', buf, ns, positions[2])
    eq(3, table.getn(rv))
    rv = buffer('get_prev_mark', buf, ns2, positions[2])
    eq(3, table.getn(rv))
    rv = buffer('get_next_mark', buf, ns, positions[1])
    eq(3, table.getn(rv))
    rv = buffer('get_next_mark', buf, ns2, positions[1])
    eq(3, table.getn(rv))

    rv = buffer('get_next_markrange', buf, ns, positions[1], positions[2])
    eq(2, table.getn(rv))
    rv = buffer('get_next_markrange', buf, ns2, positions[1], positions[2])
    eq(2, table.getn(rv))
    rv = buffer('get_prev_markrange', buf, ns, positions[1], positions[2])
    eq(2, table.getn(rv))
    rv = buffer('get_prev_markrange', buf, ns2, positions[1], positions[2])
    eq(2, table.getn(rv))
